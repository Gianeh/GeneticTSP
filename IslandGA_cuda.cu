...
#define generations 4000		// number of generations

#define pathSize 100				// dataset size in number of coordinates
#define popSize 8192				// population size
#define subPopSize 8				// popSize must be a multiple of this and this should be a multiple of the warp size (32)
...
__device__ void device_distance_crossover(int *parent1, int *parent2, int *offspring, double *distance_matrix){
	...
	curandState state;
	curand_init((unsigned long long)clock(),0,0,&state);	// init random seed
...
__device__ void device_crossover(int *sub_population, double *distance_matrix){
	...
	*/
	...
	// init the random seed
	curandState state;
	curand_init((unsigned long long)clock()+threadIdx.x,0,0,&state);	// init random seed
...
__global__ void genetic_step(int *population, double *population_fitness, double *population_distances, double *distance_matrix){
...
int main(){
    ...
    load_data(path_coordinates, "./data/krolak_coords.txt");
	...
	while(generation <= generations){
		...
		genetic_step<<<s_grid,s_block>>>(gpu_population, gpu_population_fitness, gpu_population_distances, gpu_distance_matrix); // subpopulation granularity
			checkCUDAError("genetic_step kernel launch");
			cudaDeviceSynchronize();				// IS THIS NECESSARY??
		mutation<<<c_grid,c_block>>>(gpu_population);	// chromosome granularity
	...
	clock_t end = clock();
	fprintf(stderr,"Execution completed in %.2f seconds\n", ((double) (end - start)) * 1000.0 / CLOCKS_PER_SEC);
