...
#define generations 2500		// number of generations
...
// A kernel to shuffle the chromosomes in each subpopulation
__global__ void sub_pop_random_shuffle(int *population){
	int sub_pop_index = blockIdx.x * threadsPerBlock + threadIdx.x * subPopSize * pathSize;
	// avoid calculation if island exceeds the number of islands
	if (sub_pop_index < popSize){
		// shuffle the chromosomes in the subpopulation
		for (int i = 0; i < subPopSize; i++){
			device_random_shuffle(population+sub_pop_index+i*pathSize, sub_pop_index+i);
		}
	}
}
...
__global__ void calculate_scores(int *population, double *distance_matrix,  double *population_fitness, double *population_distances){
	int sub_pop_index = blockIdx.x * threadsPerBlock + threadIdx.x * subPopSize * pathSize;
	int fitness_index = blockIdx.x * threadsPerBlock + threadIdx.x * subPopSize;
	// avoid calculation if island exceeds the number of islands
	if (sub_pop_index < popSize){
		// calculate the distance and fitness for each individual in the subpopulation
		for (int i = 0; i < subPopSize; i++){
			population_distances[fitness_index+i] = device_calculate_distance(population+sub_pop_index+i*pathSize, distance_matrix);
			population_fitness[fitness_index+i] = 10000 / population_distances[fitness_index+i];
		}
	}
}
...
__device__ void device_crossover(int *sub_population, double *distance_matrix){
	...
	// init the random seed
	curandState state;
	curand_init((unsigned long long)clock()+island,0,0,&state);	// init random seed
	...
	if (island < popSize/subPopSize){
		...
		for(int i = 1; i < subPopSize; i++){
			if (curand_uniform(&state) < mutation_rate){
				...
				int idx1 = int(curand_uniform(&state)*pathSize) % pathSize;// random from 0 to 1 * path size
...
int main(){
	...
	while(generation <= generations){
		...
		printf("Starting Mutation_3\n");
		mutation<<<s_grid,s_block>>>(gpu_population);	// subpopulation granularity
